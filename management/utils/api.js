...
// 配置
function getConfig() {
  return request({ method: 'GET', path: '/api/config' }).then(res => res.data || {})
}
...
function getInventoryIOStats(startDate, endDate) {
  ...
  }).then(res => (res.success && res.data && res.data.items ? res.data.items : []))
}
...
function putPasscode(passcode) {
  return request({ method: 'PUT', path: '/api/user/passcode', data: { passcode } })
}
